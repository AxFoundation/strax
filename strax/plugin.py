...
class RechunkerPlugin(Plugin):
    """
    This is a rather brute force plugin. It keeps a state and as such does
    not allow for parallization and other nice strax features (similar to
    the OverLapWindowPlugin). It's intended to be used for data that is sparse
    in times and is hard to fit into the chunking paradigm.
    Examples are:
     - handing logic signals, that can span minutes and are NOT sorted in
       endtime (assumed by OverlapWindowPlugin), and requite a lot of overhead
       if all chunks were buffered in an OverlapWindowPlugin.
     - Simulation plugins where one might realize that there was actually a
    ...
    def iter(self, iters, executor=None)->strax.Chunk:
        yield from super().iter(iters, executor=executor)

        if self.cached_input:
            final_chunk = self.get_final_result()
            yield final_chunk
    ...
    def _input_to_cache(self, kwargs)->None:
        """Push new input to the cache and check this is not getting out of hand"""
        for data_kind, chunk in kwargs.items():
            if data_kind in self.cached_input:
                ...
                self.cached_input[data_kind] = chunk
        tot_size = sum([c.nbytes for c in self.cached_input.items()])
        if tot_size >= self.max_buffer_mb:
            # If you are lazy, just increase the self.max_buffer_mb
            raise ValueError(f'Buffer size of {self} exceeds the '
                             f'{self.max_buffer_mb} MB limit, your data is '
                             f'either too big or too dense for the '
                             f'{self.min_gap_in_chunk()} ns gap or data')

    # Can be jitted and/or optimized
    def _find_split_in_input(self):
        """
        Find the latest time where the gap to the next start time is
        larger than the requested gap
        """
        if any(not len(c) for c in self.cached_input.values()):
    ...
    def get_final_result(self):
        """On the last chunk, just compute whatever we have left and
        clear the cache
        """
