...
def sorted_bounds(
    disjoint=False, max_value=50, max_len=10, min_size=0, max_size=20, remove_duplicates=False
):
    if disjoint:
    ...
    s = strategies.lists(
        strategies.integers(min_value=0, max_value=max_value), min_size=min_size, max_size=max_size
    )
    if disjoint:
    ...
    if disjoint:
        s = s.map(lambda x: [tuple(q) for q in iterutils.chunked(sorted(x), size=2)])
    else:
        s = s.map(lambda x: [tuple(sorted(q)) for q in iterutils.chunked(x, size=2)])
...
disjoint_sorted_intervals = sorted_bounds(disjoint=True).map(bounds_to_intervals)

fake_hits = sorted_bounds().map(partial(bounds_to_intervals, dtype=strax.hit_dtype))
...
def bounds_to_records(bs, single=False, single_channel=False):
    """Return strax records corresponding to a list of 2-tuples of boundaries.
...
single_fake_pulse = sorted_bounds().map(partial(bounds_to_records, single=True))

several_fake_records = sorted_bounds().map(bounds_to_records)

several_fake_records_one_channel = sorted_bounds(disjoint=True).map(
...
class PeaksWoPerRunDefault(strax.Plugin):
    """Same as peak plugin but without per run default option to allow for plugin caching."""
...
class PeakClassification(strax.Plugin):
    provides = ("peak_classification", "lone_hits")
    data_kind = dict(peak_classification="peaks", lone_hits="lone_hits")
    depends_on = ("peaks",)
    ...
    def infer_dtype(self):
        ...
        return {"peak_classification": peaks_dtype, "lone_hits": lone_hits}
...
@strax.takes_config(
    strax.Option(
        "by_child_overwrite_option",
        type=int,
        default=DEFAULT_CONFIG_TEST["area_parent"],
        help="Option we will overwrite in our child plugin",
    ),
    strax.Option(
        "parent_unique_option",
        type=int,
        default=DEFAULT_CONFIG_TEST["max_gap_both"],
        help="Option which is not touched by the child and therefore the same for parent and child",
    ),
    strax.Option("context_option", type=int, help="Tracked context option e.g. n_pmts_tpc."),
    strax.Option(
...
@strax.takes_config(
    strax.Option(
        "by_child_overwrite_option_child",
        type=int,
    ...
    strax.Option(
        "context_option_child",
        type=int,
        default=DEFAULT_CONFIG_TEST["area_per_channel_shape_child"][0],  # type: ignore
        child_option=True,
        parent_option_name="context_option",
        help="Tracked context option e.g. n_pmts_tpc.",
    ),
    strax.Option(
        "child_exclusive_option",
        type=int,
        default=DEFAULT_CONFIG_TEST["nhits_child"],
        help="Option which is exclusive for the child.",
    ),
    strax.Option(
        "more_special_context_option_child",
        type=immutabledict,
        child_option=True,
