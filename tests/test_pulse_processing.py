...
def test_find_hits_randomize(records):
    """Tests the hitfinder with whatever hypothesis can throw at it (well, pulse only takes (0, 1),
    and we only test a single pulse at a time)"""
    results = _find_hits(records)
    w = records[0]["data"]

    # Check for false positives
    for ll, rr in results:
        assert np.all(w[ll:rr] == 1)
...
def test_filter_waveforms():
    """Test that filter_records gives the same output as a simple convolution applied to the
    original pulse (before splitting into records)"""
