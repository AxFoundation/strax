...
from .test_cut_plugin import _dtype_name, full_dt_dtype, get_some_array
...
def drop_random(input_array: np.ndarray) -> np.ndarray:
    ...
    :param input_array: numpy array to modify. Here we will drop some of the indices in the array
        randomly.
...
def _loop_test_inner(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert np.shape(result) == np.shape(
            big_data
        ), "Looping over big_data resulted in a different datasize?!"
        assert np.sum(result[_dtype_name]) >= np.sum(
            big_data[_dtype_name]
        ), "Result should be at least as big as big_data because we added small_data data"
        assert isinstance(result, np.ndarray), "Result is not ndarray?"


@given(
    get_some_array().filter(lambda x: len(x) >= 0), strategies.integers(min_value=1, max_value=10)
)
@settings(deadline=None)
@example(
    big_data=np.array(
        [(0, 0, 1, 1), (1, 1, 1, 1), (5, 2, 2, 1), (11, 4, 2, 4)], dtype=full_dt_dtype
    ),
    nchunks=2,
)
...
def test_loop_plugin_multi_output(
    big_data,
    nchunks,
):
    """Test the loop plugin for random data where it should give multiple outputs."""
    _loop_test_inner(big_data, nchunks, target="other_combined_things")


@given(
    get_some_array().filter(lambda x: len(x) == 0), strategies.integers(min_value=2, max_value=10)
)
@settings(deadline=None)
