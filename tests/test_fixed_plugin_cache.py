...
class MaxChannelPeaks(Peaks):
    def infer_dtype(self):
        ...
        dtype = strax.peak_dtype() + [(("PMT with median most records", "max_pmt"), np.int16)]
...
class TestContextFixedPluginCache(unittest.TestCase):
    ...
    def test_load_runs_multicore(self):
        ...
        If the references are mixed up the results are inconsistent

        """
        multicore_config = dict(
            allow_lazy=False,
            timeout=60,
    ...
    def get_context(use_per_run_defaults: bool):
        """Get simple context."""
        st = strax.Context(
            storage=[], register=(ChannelIsRunidRecords, MaxChannelPeaks), config=dict(bonus_area=1)
        )
