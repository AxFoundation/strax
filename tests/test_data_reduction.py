...
def test_cut_outside_hits(records):
    ...
    records_out = strax.cut_outside_hits(
        records, hits, left_extension=left_extension, right_extension=right_extension
    )
    ...
    for x in records.dtype.names:
        ...
        if x == "reduction_level":
            np.testing.assert_array_equal(
                records_out[x],
                np.ones(len(records), dtype=np.int16) * strax.ReductionLevel.HITS_ONLY,
            )
        else:
            np.testing.assert_array_equal(records_out[x], records[x], err_msg=f"Field {x} mangled!")
    ...
    for r in records:
        for i, w in enumerate(r["data"][: r["length"]]):
            t = r["time"] + i * r["dt"]
            for h in hits:
                if h["time"] - left_extension <= t < strax.endtime(h) + right_extension:
