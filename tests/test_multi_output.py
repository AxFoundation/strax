...
class EvenOddSplit(strax.Plugin):
    ...
    data_kind = dict(even_recs="even_recs", odd_recs="odd_recs", rec_count="chunk_bonus_data")
...
class TestMultiOutputs(unittest.TestCase):
    def setUp(self):
        ...
        self.mystrax = strax.Context(
            ...
            register=[
                ...
                FunnyPeaks,
                EvenRecsClassified,
                ZipRecordsAdditionalDependency,
            ],
    ...
    def test_save_when_per_provide_same_save_when(self):
        """Tests if we can specify the save_when parameter per data_type provided by the plugin."""
    ...
    def test_double_dependency_notlazy(self):
        """Tests if double dependency of a plugin on another plugin leads to dead lock in
        processing."""
        self.mystrax.set_context_config({"timeout": 120, "allow_lazy": False})
        self._test_double_dependency()
    ...
    def test_multi_output(self):
        for max_workers in [1, 2]:
            ...
            funny_ps = self.mystrax.get_array(
                run_id=run_id, targets="peaks", max_workers=max_workers
            )
