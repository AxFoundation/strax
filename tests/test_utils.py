...
def test_time_selection(d, second_time, second_dt):
    """Test that both 'touching' and 'fully_contained' give the same results as
    'strax.fully_contained_in' and 'strax.touching_windows' respectively.

    :param d: test-data from get_dummy_data
    :param second_dt: the offset w.r.t. the first
    :return: None
    ...
    # Fully contained in
    selected_data = strax.apply_selection(
        d, time_range=time_range, time_selection="fully_contained"
    )
    ...
    # TW
    selected_data = strax.apply_selection(d, time_range=time_range, time_selection="touching")
...
class TestMultiRun(unittest.TestCase):
    ...
    def test_multi_run_memory_profile(self):
        """Tests if multi-runs fills up memory.

        Use only a single worker hence there should not be at any time more than 2 runs inside the
        ThreadPoolExecutor.
        ...
        mem_overhead = memory_usage((self.context.make, ("1", "records")))
        mem_overhead = np.mean(mem_overhead)

        # Now get size of a single array:
        size_per_run = self.context.size_mb("1", "records")
        self.context.get_array("1", "records")
        # Make 50 runs and compare memory usage:
        used_mem = memory_usage((self.context.make, ([str(r) for r in range(50)], "records")))
