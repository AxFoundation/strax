...
class TestMongoFrontend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mongo_sf = strax.MongoFrontend(
            uri=uri,
            database=db_name,
    ...
    def test_write_and_load(self):
        ...
        msg = (
            "since take_only of the MongoFrontend is specified not all data "
    ...
    def test_write_and_change_lineage(self):
        """Lineage changes should result in data not being available and therefore the data should
        not be returned."""
    ...
    def test_clean_cache(self):
        """We keep a small cache in the backend of the last loaded data for offloading the database,
        test that it works."""
        self._make_mongo_target()
        assert self.is_stored_in_mongo
    ...
    def test_interrupt_iterator(self):
        """When we interrupt during the writing of data, make sure we are not able to data that is
        only half computed."""
        assert not self.is_stored_in_mongo
    ...
    def test_allow_incomplete(self):
        ...
        if not self.stored_in_context(st_incomplete_allowed):
            raise RuntimeError(
                "We did not find the one chunk that should be "
        ...
        if not stored_in_st or not stored_in_incomplete_st:
            raise RuntimeError(
                "Source finished and should be stored in st "
    ...
    def test_allow_incomplete_during_md_creation(self):
        """Test that allowing incomplete data does not find data if the metadata ("md") is just
        created.

        See #596 for more info

        Test is different from "test_allow_incomplete" in the sense that no chunks are written at
        all, only the md is registered
        ...
        if stored_in_st or stored_in_incomplete_st:
            raise RuntimeError(
                "Only metadata written and should NOT stored in st "
                f"({stored_in_st}) and st_incomplete ("
                f"{stored_in_incomplete_st}) "
                f"\n{self.chunk_summary}"
            )
