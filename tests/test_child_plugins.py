...
def test_child_plugin_config():
    """Test if options of parent plugin are correctly replaced by config options of the child
    plugin. The following things are test:
    ...
    mystrax = strax.Context(
        storage=[],
        config={
            "context_option": DEFAULT_CONFIG_TEST["area_per_channel_shape_parent"][0],
            "more_special_context_option": immutabledict(
                tpc=DEFAULT_CONFIG_TEST["channel_map_parent"]
            ),
        },
    ...
    mes = (
        "The channel identfication did not work for the child",
        'This means "more_special_context_option_child" did not work properly',
    )
    ...
    m = np.all(
        (parent["max_gap"] == new_test_settings["parent_unique_option"])
...
def test_child_plugin_lienage():
    ...
    hp, hc = _check_hash("child_exclusive_option", hp, hc, False, True)

    # Parent exclusive option:
    mystrax.set_config({"by_child_overwrite_option": 42})
    hp, hc = _check_hash("parent_exclusive_option", hp, hc, True, False)

    # Parent unique option (shared by child and arent):
    mystrax.set_config({"parent_unique_option": 33})
    hp, hc = _check_hash("child_exclusive_option", hp, hc, True, True)

    # Context option parent:
    mystrax.set_config({"context_option": 33})
    hp, hc = _check_hash("child_exclusive_option", hp, hc, True, False)
