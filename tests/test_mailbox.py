...
def mailbox_tester(
    ...
    with concurrent.futures.ThreadPoolExecutor() as tp:
        futures = [
            tp.submit(reader, source=mb.subscribe(), reader_sleeps=reader_sleeps)
        ...
        for f in futures:
            np.testing.assert_equal(f.result(timeout=result_timeout), expected_result)
...
def test_write_timeout():
    ...
    with pytest.raises(strax.MailboxFullTimeout):
        mailbox_tester([0, 1, 2, 3, 4], max_messages=1, reader_sleeps=LONG_TIMEOUT)
...
def test_deadlock_regression():
    ...
    readers = [
        threading.Thread(target=reader, kwargs=dict(source=mb.subscribe(), name=str(i)))
        for i in range(2)
    ]
    readers[0].start()
    time.sleep(SHORT_TIMEOUT)
