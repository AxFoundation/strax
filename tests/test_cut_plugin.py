...
@given(
    get_some_array().filter(lambda x: len(x) >= 0), strategies.integers(min_value=-10, max_value=10)
)
@settings(deadline=None)
# Examples for readability
@example(
    input_peaks=np.array(
        [(-11, 0, 1), (0, 1, 3), (-5, 3, 5), (11, 5, 7), (7, 7, 9)],
...
@example(
    input_peaks=np.array(
        [(0, 0, 1, 1), (1, 1, 1, 1), (5, 2, 2, 1), (11, 4, 2, 4)],
...
def test_cut_plugin(input_peaks, cut_threshold):
    ...
    result = st.get_array(run_id="some_run", targets=strax.camel_to_snake(CutSomething.__name__))
    correct_answer = np.sum(input_peaks[_dtype_name] > cut_threshold)
    assert len(result) == len(input_peaks), "WTF??"
    assert correct_answer == np.sum(
        result["cut_something"]
    ), "Cut plugin does not give boolean arrays correctly"

    if len(input_peaks):
        assert (
            strax.endtime(input_peaks).max() == strax.endtime(result).max()
        ), "last end time got scrambled"
        assert np.all(input_peaks["time"] == result["time"]), "(start) times got scrambled"
        assert np.all(
            strax.endtime(input_peaks) == strax.endtime(result)
        ), "Some end times got scrambled"
