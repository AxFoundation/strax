...
def get_int_array(min_value=0, max_value=1, min_size=0, max_size=20) -> strategies.lists:
    """Get array with ints.
...
def get_float_array(min_value=0, max_value=1, min_size=0, max_size=20):
    ...
    """
    return strategies.lists(
        strategies.floats(min_value=min_value, max_value=max_value),
        min_size=min_size,
        max_size=max_size,
    )
...
def _test_splitter_inner(min_heights, min_ratios, waveform, splitter):
    ...
    for min_height, min_ratio in zip(min_heights, min_ratios):
        ...
        if splitter == "local_minimum":
            my_splits = test_splitter.find_split_points(
                waveform, dt=None, peak_i=None, min_height=min_height, min_ratio=min_ratio
            )
        ...
        split_checks = [(int(split - 1), int(split + 1), int(split)) for split in my_splits[:, 0]]
