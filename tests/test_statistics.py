...
def _test_highest_density_region(distribution, truth_dict):
    intervals, heights = strax.highest_density_region(
        distribution,
    ...
    for fraction_ind, (key, values) in enumerate(truth_dict.items()):
        for ind_interval, interval in enumerate(values):
            ...
            mes = (
                f"Have not found the correct edges for a fraction of {key}% found {int_found}, but"
                f" expected {interval}"
            )
...
def test_too_small_buffer():
    ...
    distribution[::4] = 0
    indicies, _ = strax.highest_density_region(
        distribution,
        np.array([0.5]),
        only_upper_part=True,
    )
    ...
    width = highest_density_region_width(
        distribution, fractions_desired=np.array([0.5]), _buffer_size=10
    )
    assert np.all(np.isnan(width))


def test_true_hdr():
    """Tests if highest density region returns for a normal distribution the expected -1/1
    boundaries for 68.27% coverage.

    We are not using a too high precision here to reduce the total test time.
